...
export const EntitiesGrid = () => {
  ...
  const getPropDisplayFields = (ent: TableEntity) => {
    return config.propertiesSelectedFields.reduce((acc: Record<string, unknown>, field: string) => {
  ...
  const onCardClicked = async (entity: ViewedTableEntity) => {
    try {
      ...
      if (entity.center_pixel) {
        ...
        const convertedPoint = await mutation.mutateAsync({
    ...
    return (
      <div className="w-full h-full flex items-center justify-center p-4">
        <div className="text-center">
          <div className="text-2xl font-semibold text-gray-600 mb-2">בחר מהות לסינון</div>
          <div className="text-sm text-gray-500">בחר מהות מהרשימה המוצגת לסינון הישויות</div>
        </div>
      </div>
    );
  }
  ...
  if (!entities || entities.length === 0) {
    return (
      <div className="w-full h-full flex items-center justify-center p-4">
        <div className="text-center">
          <div className="text-2xl font-semibold text-gray-600 mb-2">לא נמצאו ישויות</div>
          <div className="text-sm text-gray-500">לא נמצאו ישויות מתאימות למהות שנבחרה</div>
        </div>
      </div>
    );
  }

  const getCenter = (entity: ViewedTableEntity): [number, number] => {
    if (entity.center_pixel) {
      return [entity.center_pixel.x, entity.center_pixel.y];
    }
    return center(entity.polygon).geometry.coordinates as [number, number];
  };
        ...
        {entities?.map(entity => (
          ...
          >
            <div className="w-[15.625vw] h-[15.625vw]">
              <EntityMap imageId={entity.imageId} center={getCenter(entity)} className="w-full h-full rounded-t-lg" />
            </div>
            <div className="bg-purple-200 border-t border-purple-300 rounded-b-lg px-2 py-2 flex flex-col items-end">
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="text-xs text-black w-full flex text-right">
                      <span className="font-bold ml-1">{config.propertyLabels[config.entityHeaderProperty]}:</span>
                      <span className="font-bold ml-1 overflow-hidden line-clamp-2 whitespace-nowrap text-ellipsis">
                ...
                {config.propertiesSelectedFields
                  .filter((f: string) => f !== config.entityHeaderProperty)
                  .map((field: string) => (
                    <Tooltip key={field}>
                      <TooltipTrigger asChild>
                        <div className="text-xs text-black w-full flex text-right">
                          <span className="ml-1">{config.propertyLabels[field]}:</span>
                          <span className="ml-1 overflow-hidden line-clamp-2 whitespace-nowrap text-ellipsis">
